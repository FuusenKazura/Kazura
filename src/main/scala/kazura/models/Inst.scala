...
object Inst {
  ...
  object Ld   extends Inst { val op="b1010".U; val alu_op=ALUOP.ADD.U; val is_jump=false.B; val is_branch=false.B; val rf_w=false.B; val mem_r=true.B;  val mem_w=false.B; val rs1_use=false.B; val rs2_use=true.B;  val source=Seq(Source1.DISP6U.U, Source2.RS.U)}
  object St   extends Inst { val op="b1011".U; val alu_op=ALUOP.ADD.U; val is_jump=false.B; val is_branch=false.B; val rf_w=false.B; val mem_r=false.B; val mem_w=true.B;  val rs1_use=true.B;  val rs2_use=true.B;  val source=Seq(Source1.DISP6U.U, Source2.RS.U)}
  object Beq  extends Inst { val op="b1100".U; val alu_op=ALUOP.BEQ.U;  val is_jump=false.B; val is_branch=true.B; val rf_w=false.B; val mem_r=false.B; val mem_w=false.B; val rs1_use=true.B;  val rs2_use=true.B;  val source=Seq(Source1.RD.U,     Source2.RS.U)}
  object Bgt  extends Inst { val op="b1101".U; val alu_op=ALUOP.BGT.U;  val is_jump=false.B; val is_branch=true.B; val rf_w=false.B; val mem_r=false.B; val mem_w=false.B; val rs1_use=true.B;  val rs2_use=true.B;  val source=Seq(Source1.RD.U,     Source2.RS.U)}
